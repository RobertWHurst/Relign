...
const parallelLimitMap = require('../parallel-limit-map');


describe('parallelLimitMap(items, worker(item) -> promise(val)) -> promise(val)', () => {

  it('processes item arrays, executing the worker on each item and resolves the mapped results', () => {
    const items = [1, 2, 3, 4];
    return parallelLimitMap(items, i => setTimeout(() => i + 1, 10), 2).then(d =>
